...
**Implementation rule**: Every task starts unchecked. Do not create, move, or recreate `v1.1.0`, `v1.1.1`, or `v1.1.2`, create `v1.1.3`, bump the real release version, push tags, add CI, or add npmjs/custom-installer work in these implementation tasks.
...
- [X] T014 [US2] Change `src/core/update/npm.ts` `buildPackageSpec()` to return the validated HTTPS Git package spec, and `buildNpmArgs()` to return `install`, `-g`, `--ignore-scripts`, `--allow-git=all`, `--install-links=true`, and that package spec in exact order.
...
- [X] T031 [P] Update current SPEC-011 research and installation contract to record v1.1.0/v1.1.1/v1.1.2 incidents, HTTPS transport portability, credential independence, npm `>=11.9 <12`, and intended-but-not-created `v1.1.3` wording.

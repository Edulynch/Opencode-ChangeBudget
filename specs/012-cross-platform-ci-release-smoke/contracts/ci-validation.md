...
Each job explicitly selects Node.js `24.18.0` and npm `11.16.0`, then runs root-level `npm ci`, `npm run typecheck`, `npm run compile`, `npm test`, `npm pack --dry-run --json --ignore-scripts`, required/forbidden runtime tracking, runtime zero-drift validation, `npm run ci:release-gate`, and `git diff --check`.

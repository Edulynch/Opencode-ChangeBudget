...
The workflow does not create, move, delete, repair, or push tags and does not publish GitHub Releases. A platform failure leaves the immutable tag unchanged and requires a new patch release. The legitimate immutable `v1.1.4` tag failed on both smoke platforms; the harness fix is recorded in commit `1f74735`, and normal CI for that fix is green. The legitimate immutable `v1.1.5` tag passed Ubuntu and Windows tagged smoke in run `32627687768`, satisfying T032; SPEC-012 is **COMPLETE**. “Public tag” means a real repository tag pushed to GitHub, not a public repository. Existing v1.1.4 failure evidence and manual v1.1.3 evidence remain preserved as historical evidence. If the repository becomes public later, authentication can be removed without changing the canonical package spec, npm argv, or smoke assertions.

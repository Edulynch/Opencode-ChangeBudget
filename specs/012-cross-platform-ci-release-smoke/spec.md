...
4. **Given** the smoke project initially contains unrelated `opencode.json` fields, instruction files, and `AGENTS.md`, **when** integration runs, **then** all those files remain byte-identical while the native V2 wrapper is added and linked to the installed Runtime Guard.
...
- **FR-016**: Release smoke MUST verify that `AGENTS.md`, unrelated `opencode.json` fields, and existing instruction files remain byte-identical, the native V2 wrapper exists, and the wrapper references the installed Runtime Guard.

...
If either tag-smoke job fails, the tag remains unchanged and no release should be published or recommended. A new patch tag is required. SPEC-012 final acceptance remains pending until a future real tag event in this private repository passes both jobs; “public tag” means a real repository tag pushed to GitHub, not a public repository. Existing manual v1.1.3 evidence does not count. T032 remains unchecked.

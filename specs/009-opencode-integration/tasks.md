...
- [x] T006 [FR-005,FR-006,FR-024] Implement pre-flight inspection in `src/core/integration/opencode.ts`: `inspectIntegration(projectRoot, runtimeGuardEntryPath)` → classify all 3 managed resources (wrapper, instructions, opencode.json), validate Runtime Guard target exists, collect all conflicts, return `PreflightPlan` with `readyToWrite = conflicts.length === 0 && runtimeGuardTargetExists && opencodeConfig.valid`. If any CONFLICT → zero writes allowed. FR-005, FR-006, FR-024, SC-004.

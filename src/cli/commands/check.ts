...
interface PublicBaselineSummary {
  ...
  readonly excludedUnchangedCount: number;
  readonly detectedDeltaCount: number;
  ...
  if (draftPath) {
    ...
    try {
      ...
      const changedItems = await collectChangedItems(repositoryRoot, baseRevision);
       ...
       return withBaselineSummary(evaluateBudgetCheck(
      ...
      const stackPolicy = await getStackPolicyResolution(repositoryRoot, parsed.normalizedContract);
       ...
       const baseline = await reloadBaselineEvidence(repositoryRoot, payload);
        if (baseline.kind === 'unsafe') {
          return withBaselineSummary(buildFailureResult(
            'active',
           contractId,
           parsed.normalizedContract.base_revision,
            baseline.reasonCodes[0],
            `Baseline evidence is unsafe: ${baseline.reasonCodes.join(', ')}`,
          ), {
        ...
        if (baseline.kind === 'ready') {
          ...
          if (!projection.ok) {
            return withBaselineSummary(buildFailureResult(
              ...
              `Baseline comparison cannot prove a safe result: ${projection.reasonCode}`,
            ), {
          ...
          const evaluated = evaluateBudgetCheck(
            buildContractEvaluationInput(
              'active',
              contractId,
              parsed.normalizedContract,

...
import type { BaselineObjectType, BaselineObservation } from './types.js';
...
function malformed(recordIndex: number, message: string, record: string): never {
...
function splitFields(record: string, fieldCount: number, recordIndex: number): readonly string[] {
  const fields: string[] = [];
  let start = 0;
...
function objectTypeForMode(mode: string, recordIndex: number, record: string): BaselineObjectType {
...
function parseStatusPair(value: string, recordIndex: number, record: string): readonly [string, string] {
...
function parseRenameOrCopyRecord(
  ...
  if (!sourcePath) return malformed(recordIndex, 'missing rename or copy source path', record);
...
export function parseGitObservationZOutput(input: GitObservationInput): readonly BaselineObservation[] {
  const binaryPaths = new Set((input.binaryPaths ?? []).map(normalizeRepositoryPath));
  const records: BaselineObservation[] = [];

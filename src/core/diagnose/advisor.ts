import {
  ...
  NORMAL_MAX_DECLARED_PATHS,
  NORMAL_MAX_TRACKED_FILES,
  ObservableSignals,
  RecommendationReason,
  TINY_MAX_DECLARED_PATHS,
  TINY_MAX_TRACKED_FILES,
} from '../../models/diagnose.js';

export function buildReasons(signals: ObservableSignals): RecommendationReason[] {
  const reasons: RecommendationReason[] = [{ signal: 'declared_paths', value: signals.declared_path_count }];
...
export function evaluateRecommendation(
  ...
  const hasHighRiskCategory = signals.sensitive_categories.some((category) =>
  ...
  if (
    tracked <= TINY_MAX_TRACKED_FILES &&

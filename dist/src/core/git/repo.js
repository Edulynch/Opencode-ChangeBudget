...
export function runGit(repositoryRoot, args) {
    return new Promise((resolve, reject) => {
        ...
        child.on('close', (code) => {
            if (code !== 0) {
                reject(new GitEnvironmentError(`Git command failed with exit code ${code}`, {
                    args,
                    repositoryRoot,
                    exitCode: code,
                    stderr: stderr.trim(),
...
export async function getRepositoryRoot(repositoryRootHint) {
...
export async function isGitRepository(repositoryRootHint) {
    try {
...
export async function ensureGitRepository(repositoryRootHint) {
...
export async function validateRevision(repositoryRoot, revision) {

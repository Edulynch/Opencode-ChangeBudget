...
import { STACK_PROFILES, } from '../../models/change-contract.js';
import { getStackPolicyOverridesFilePath, readJsonFileOptional, } from '../state/state.js';
...
const STACK_RULE_SEVERITIES = ['review', 'deny'];
function isStackProfileValue(value) {
...
function sortByRuleId(rules) {
    return [...rules].sort((left, right) => left.id.localeCompare(right.id));
}
function normalizeRuleIdSet(value) {
    const seen = new Set();
    const normalized = [];
...
function parseRuleArray(value, profileId) {
    if (value === undefined || value === null) {
        return [];
    }
    if (!Array.isArray(value)) {
    ...
    for (let index = 0; index < value.length; index += 1) {
        ...
        if (rawProfileId !== undefined) {
            ...
            if (!isStackProfileValue(normalizedProfile)) {
                throw new InputValidationError(`Invalid added_rules[${index}].profile_id '${normalizedProfile}'`, 'added_rules', {
                    field: `added_rules[${index}].profile_id`,
                    value: normalizedProfile,
                });
            }
            if (normalizedProfile !== profileId) {
                throw new InputValidationError(`added_rules[${index}].profile_id must match selected profile '${profileId}'`, 'added_rules', {
                    field: `added_rules[${index}].profile_id`,
                    expected: profileId,
        ...
        if (typeof rawCategory !== 'string' || !STACK_RULE_CATEGORIES.includes(rawCategory)) {
            throw new InputValidationError(`Invalid added_rules[${index}].category`, 'added_rules', {
                field: `added_rules[${index}].category`,
                value: rawCategory,
            });
        }
        const rawSeverity = candidate.severity;
        if (typeof rawSeverity !== 'string' || !STACK_RULE_SEVERITIES.includes(rawSeverity)) {
            throw new InputValidationError(`Invalid added_rules[${index}].severity`, 'added_rules', {
        ...
        if (!message.length) {
            throw new InputValidationError(`added_rules[${index}].message must be a non-empty string`, 'added_rules', {
        ...
        if (targetPatterns.length === 0) {
            throw new InputValidationError(`added_rules[${index}].target_patterns must include at least one pattern`, 'added_rules', {
...
function parseProfileOverride(profileId, payload) {
    if (!isObject(payload)) {
...
function parseOverrideFile(payload) {
    if (!isObject(payload)) {
    ...
    if (!isObject(payload.profiles)) {
        throw new InputValidationError('Invalid stack policy override file; profiles must be an object', 'stack-policy-overrides', {
            field: 'profiles',
        });
    }
    const profiles = {};
    for (const [candidateProfileId, profilePayload] of Object.entries(payload.profiles)) {
        if (!isStackProfileValue(candidateProfileId)) {
            throw new InputValidationError(`Unsupported stack profile key '${candidateProfileId}' in profiles`, 'stack-policy-overrides', {
                field: 'profiles',
                value: candidateProfileId,
            });
        }
        profiles[candidateProfileId] = profilePayload;
    }
    result.profiles = profiles;
    return result;
}
export function getBuiltInStackProfileRules(profileId) {
    const rules = BUILTIN_RULES[profileId];
    if (!rules) {
        throw new InputValidationError(`No built-in rules for profile '${profileId}'`, 'stack_profile', { profile: profileId });
    }
    return sortByRuleId(rules);
}
function getOverrideForProfile(repositoryRoot, profileId) {
    return readJsonFileOptional(getStackPolicyOverridesFilePath(repositoryRoot))
        .catch((error) => {
        if (error instanceof StateCorruptionError) {
            throw new InputValidationError('Invalid JSON in stack policy override file', 'stack-policy-overrides', {
...
function validateRulePatterns(profileId, rules) {
...
export async function resolveStackPolicy(repositoryRoot, profileId, contractDisabledRuleIds) {
    if (!profileId) {
        return null;
    }
    ...
    const seenRuleIds = new Set(builtinRules.map((entry) => entry.id));
    ...
    for (const rule of repositoryAddedRules) {
        if (seenRuleIds.has(rule.id)) {
            throw new InputValidationError(`Repository override adds duplicate rule id '${rule.id}' already in profile '${normalizedProfileId}'`, 'added_rules', {
    ...
    const normalizedContractDisabled = normalizeRuleIdSet(contractDisabledRuleIds.map((entry) => entry.trim()));
...
export function buildStackReasonCode(ruleId) {
    const normalized = ruleId.trim();
    return `CBS-${normalized.toUpperCase().replace(/\//g, '-')}`;
}
//# sourceMappingURL=stack-policy.js.map
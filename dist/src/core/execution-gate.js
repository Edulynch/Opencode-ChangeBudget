...
function evaluateFastPath(envelope, operation) {
    if (envelope.satisfaction.state === 'OPEN')
        return { kind: 'NON_MATERIAL' };
    switch (operation) {
        case 'read_only':
        case 'introspection':
...
function evaluateMaterialDecision(envelope, proposal) {
    const structuralFailure = validateProposal(envelope, proposal);
    if (structuralFailure !== undefined)
        return invalidProposal(structuralFailure);
...
function validateProposal(envelope, proposal) {
...
function validateNumericProposal(proposal, authority) {
    ...
    if (proposal.necessity === 'optional')
        return undefined;
...
function validateAllowlistAuthority(proposal, authority, criterionIds) {
...
function evaluateNumericProposal(proposal, authority) {
    if (proposal.requested.amount <= authority.max)
        return governance('APPROVE', 'Requested numeric authority is declared by the envelope');
    ...
    if (authority.max > 0)
        return governance('REDUCE', 'Requested numeric authority exceeds the declared maximum', authority.max);
...
function evaluateAllowlistProposal(envelope, proposal, authority) {
    if (authority.allowed.includes(proposal.requested.value))
        return governance('APPROVE', 'Requested value is declared by the envelope');
    ...
    if (proposal.necessity === 'required')
        return governance('ESCALATE', 'Required value is outside the declared allowlist');
    return authority.constraint === 'HARD'
...
function isCriterionSatisfied(envelope, criterionId) {
...
function governance(verdict, reason, reducedAmount, replacementValue) {
...
function assertNeverReason(value) {

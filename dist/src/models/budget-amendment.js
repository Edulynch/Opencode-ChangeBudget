...
export const BUDGET_AMENDMENT_FIELDS = ['max_files', 'max_changed_lines'];
...
function parseBudgetLimit(value, field, context) {
    if (value === null) {
        return null;
    }
...
function parseLimitChange(value, field, context) {
    if (!isObject(value)) {
...
function parseChanges(value, context) {
    if (!isObject(value)) {
...
function assertCurrentLimit(value, field, contractId) {
    parseBudgetLimit(value, field, { contractId });
}
export function validateBudgetAmendments(value, contractId, limits) {
...
//# sourceMappingURL=budget-amendment.js.map
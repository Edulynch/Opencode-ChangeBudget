...
function printDiagnoseResultJson(result) {
    const payload = {
...
function printCheckResult(result) {
    ...
    if (result.violations.length > 0) {
        ...
        for (const violation of result.violations) {
            ...
            stdout.write(`  - ${violation.rule}: ${violation.message} [path=${path}, expected=${expected}, observed=${observed}, reason_code=${reasonCode}, action=${action}]\n`);
        }
    }
    else {
...
function printStatusResultJson(result) {
    if (result.budgetResult) {
        printCheckResultJson(result.budgetResult);
    }
}
function printStatusBudgetResult(result) {
    ...
    switch (command) {
        ...
        case 'update': {
            if (args.length > 1 || (args.length === 1 && args[0] !== '--check')) {
                throw new InputValidationError('Unsupported update flag', 'update', { args });
            }

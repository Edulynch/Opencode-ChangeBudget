...
function takeValue(args, index, inlineValue) {
...
function canonicalizeAmendPath(value) {
    ...
    if (lower === '.changebudget' || lower.startsWith('.changebudget/'))
        return null;
    return normalized;
}
...
function classifyDiagnose(args) {
    let taskIdSeen = false;
    for (let index = 0; index < args.length; index += 1) {
        const token = args[index];
        if (!token.startsWith('--')) {
            if (taskIdSeen || !/^[Tt]\d{3,}$/.test(token))
                return 'unsupported';
            taskIdSeen = true;
            continue;
        }
        ...
        if (!taken)
            return 'unsupported';
        if (key === 'stack-profile' && !STACK_PROFILES.has(taken.value.toLowerCase()))
            return 'unsupported';
        index = taken.nextIndex;
    }
    return 'read-only';
}
function classifyCheck(args) {
    let draftSeen = false;
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (!READ_ONLY_CHECK_FLAGS.has(key) && key !== 'satisfaction-evidence-json')
            return 'unsupported';
        if (key === 'draft' && draftSeen)
            return 'unsupported';
        if (key === 'satisfaction-evidence-json' && evidenceSeen)
            return 'unsupported';
        const taken = takeValue(args, index, inlineValue);
        if (!taken)
            return 'unsupported';
        if (key === 'satisfaction-evidence-json' && !isValidSatisfactionEvidence(taken.value))
            return 'unsupported';
        index = taken.nextIndex;
        if (key === 'draft')
            draftSeen = true;
    ...
    if (draftSeen && evidenceSeen)
        return 'unsupported';
    return evidenceSeen ? 'managed-mutation' : 'read-only';
}
function isValidSatisfactionEvidence(value) {
    let payload;
    try {
...
function classifyStart(args) {
    ...
    for (let index = 0; index < args.length; index += 1) {
        const token = args[index];
        if (!token.startsWith('--')) {
            if (positionalTask || !/^[Tt]\d{3,}$/.test(token))
                return 'unsupported';
            positionalTask = true;
            taskSupplied = true;
            continue;
        ...
        if (['tiny', 'normal', 'free'].includes(key)) {
            if (inlineValue !== null || presetSpecified)
                return 'unsupported';
            presetSpecified = true;
            continue;
        }
        if (!START_VALUE_OPTIONS.has(key))
            return 'unsupported';
        if (inlineValue !== null && inlineValue.length === 0)
            return 'unsupported';
        const taken = takeValue(args, index, inlineValue);
        if (!taken)
            return 'unsupported';
        if (key === 'task' || key === 'task-description')
            taskSupplied = taken.value.trim().length > 0;
        if (key === 'max-files' || key === 'max-changed-lines') {
            if (!isNonNegativeInteger(taken.value))
                return 'unsupported';
        }
        if (key === 'preset' && !CONTRACT_PRESETS.has(taken.value.toLowerCase()))
            return 'unsupported';
        if (key === 'preset') {
            if (presetSpecified)
                return 'unsupported';
        ...
        if (key === 'execution-envelope-json') {
            if (executionEnvelopeSeen || !isJsonObject(taken.value))
                return 'unsupported';
            executionEnvelopeSeen = true;
        }
        if (['allow-path', 'allow-paths', 'deny-path', 'deny-paths'].includes(key)
            && !taken.value.split(',').every((entry) => entry.trim().length > 0))
            return 'unsupported';
        if (['disable-stack-rule', 'disable-stack-rules'].includes(key)
            && !taken.value.split(',').some((entry) => entry.trim().length > 0))
            return 'unsupported';
        index = taken.nextIndex;
    }
    return taskSupplied ? 'managed-mutation' : 'unsupported';
}
function classifyAmend(args) {
    let hasMutation = false;
    let hasAllowPath = false;
    let hasReason = false;
    const seen = new Set();
    const seenAllowPaths = new Set();
    for (let index = 0; index < args.length; index += 1) {
        const token = args[index];
        if (!token.startsWith('--'))
            return 'unsupported';
        const { key, value: inlineValue } = parseOption(token);
        if (!['max-files', 'max-changed-lines', 'allow-path', 'allow-paths', 'reason'].includes(key))
            return 'unsupported';
        if (key !== 'allow-path' && key !== 'allow-paths' && seen.has(key))
            return 'unsupported';
        ...
        if (!taken)
            return 'unsupported';
        if ((key === 'max-files' || key === 'max-changed-lines') && !isNonNegativeInteger(taken.value))
            return 'unsupported';
        if (key === 'reason') {
            if (taken.value.trim().length === 0)
                return 'unsupported';
            hasReason = true;
        }
        if (key === 'allow-path' || key === 'allow-paths') {
            if (!taken.value.split(',').every((entry) => entry.trim().length > 0))
                return 'unsupported';
            for (const entry of taken.value.split(',')) {
                const canonical = canonicalizeAmendPath(entry);
                if (canonical === null || seenAllowPaths.has(canonical))
                    return 'unsupported';
...
function classifyClose(args) {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (!token.startsWith('--'))
            return 'unsupported';
        const { key, value: inlineValue } = parseOption(token);
        if (!['force', 'actor', 'reason'].includes(key) || seen.has(key))
            return 'unsupported';
        seen.add(key);
        if (key === 'force') {
            if (inlineValue !== null)
...
function classifyIntegrate(args) {
    if (args[0] !== 'opencode')
        return 'unsupported';
    let dryRun = false;
    let remove = false;
    for (const token of args.slice(1)) {
        if (token === '--dry-run' && !dryRun)
            dryRun = true;
        else if (token === '--remove' && !remove)
            remove = true;
        else
            return 'unsupported';
    }
